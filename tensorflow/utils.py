...
def symbol_error_rate(xBatch, xHatBatch):
    ...
    for k in np.arange(0, batch_size):
        ...
        correct_k = tf.reduce_sum(tf.cast(tf.equal(xBatch[k], xHatBatch[k]), tf.int32))
        correct_k = tf.cast(correct_k/NR, tf.float32)
        correct += correct_k
...
def plot_fig(args, SNRdB, SER_list, save_path):
    ...
    plt.xlabel('SNR(dB)')
    plt.ylabel('SER')
    template = 'NT{:d}_NR{:d}_'.format(args.NT, args.NR) + args.modulation + '_epochs_' + str(args.epochs)
    plt.grid(template)
